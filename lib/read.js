...
export async function readHeader(handle) {
	...
	header.data_hash_table_offset = ref(buffer.readBigUInt64LE(104), /** @type {import('./index.d.ts').Reader<import('./index.d.ts').HashTableItem<import('./index.d.ts').DataObjectPayload>, [bigint]>} */(readHashTableItem(handle, header)));
	header.data_hash_table_size = buffer.readBigUInt64LE(112);
	// eslint-disable-next-line max-len
	header.field_hash_table_offset = ref(buffer.readBigUInt64LE(120), /** @type {import('./index.d.ts').Reader<import('./index.d.ts').HashTableItem<import('./index.d.ts').FieldObjectPayload>, [bigint]>} */(readHashTableItem(handle, header)));
 ...
 *   import('./index.d.ts').HashTableItem<ContainedObjectPayload>,
 *   [bigint]
 * >}
 */
export function readHashTableItem(handle, header) {
	const specializedReadObject = readObject(handle, header);

	return async (offset, index) => {
		assert(header.data_hash_table_size <= Number.MAX_SAFE_INTEGER);
		assert((header.data_hash_table_size & 0xfn) === 0n);
		...
		return {
			// eslint-disable-next-line max-len
			head_hash_offset: nullableRef(buffer.readBigUInt64LE(0), /** @type {import('./index.d.ts').ObjectReader<ContainedObjectPayload>} */(specializedReadObject)),
			// eslint-disable-next-line max-len
			tail_hash_offset: nullableRef(buffer.readBigUInt64LE(8), /** @type {import('./index.d.ts').ObjectReader<ContainedObjectPayload>} */(specializedReadObject))
		};
	};
}
...
export function readDataObjectPayload(handle, header, object) {
	...
	return async offset => {
		...
		if (object.flags & objectFlags.COMPRESSED_XZ) {
			...
			payload.payload = Buffer.from(await decompress(payload.payload));
		}
