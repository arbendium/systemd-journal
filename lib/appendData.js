...
export default async function appendData(handle, header, content) {
	...
	await handle.writev([object, content], /** @type {any} */(offset));
	...
	await handle.write(offsetBuffer, /** @type {any} */(field.payload.offset + 16n));
	...
	await handle.write(object.subarray(32, 40), /** @type {any} */(offset + 32n));
...
async function linkData(handle, header, hash, offset) {
	...
	if (tail == null) {
		await handle.writev(
			[buffer, buffer],
			/** @type {any} */(header.data_hash_table_offset.offset + hashTablePosition * 16n)
		);
	} else {
		// next_hash_offset
		await handle.write(
		...
		await handle.write(
			buffer,
			/** @type {any} */(header.data_hash_table_offset.offset + hashTablePosition * 16n + 8n)
		);
	}
}
