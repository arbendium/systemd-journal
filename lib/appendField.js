...
async function linkField(handle, header, hash, offset) {
	const hashTablePosition = Number(hash % (header.field_hash_table_size / 16n));
	...
	if (tail == null) {
		await handle.writev(
			[buffer, buffer],
			Number(header.field_hash_table_offset.offset) + (hashTablePosition * 16)
		);
	} else {
		// next_hash_offset
		await handle.write(
		...
		await handle.write(
			buffer,
			Number(header.field_hash_table_offset.offset) + (hashTablePosition * 16) + 8
		);
	}
}
