...
function serializeHeader(header) {
	const realHeaderSize = header.header_size;

	if (realHeaderSize !== BigInt(headerSize)) {
		throw new Error(`Invalid header size: ${header.header_size}`);
	}

	assert(realHeaderSize <= Number.MAX_SAFE_INTEGER);

	const buffer = Buffer.allocUnsafe(Number(realHeaderSize));
	...
	if (isHeader187(header)) {
		buffer.writeBigUInt64LE(header.n_data, 208);
		buffer.writeBigUInt64LE(header.n_fields, 216);
	}

	if (isHeader189(header)) {
		buffer.writeBigUInt64LE(header.n_tags, 224);
		buffer.writeBigUInt64LE(header.n_entry_arrays, 232);
	}

	if (isHeader246(header)) {
		buffer.writeBigUInt64LE(header.data_hash_chain_depth, 240);
		buffer.writeBigUInt64LE(header.field_hash_chain_depth, 248);
	}
